...
public class MazeNodeA extends Node<Action> {
    public static int endx=9,endy=9;
    public MazeNodeA(MazeState currentState, Node<Action> parentNode, Action action) {
    ...
    public int compareTo(Node<Action> o) {
        return (int) (cost + (((MazeState) currentState).getX()-endx)*(((MazeState) currentState).getX()-endx) +(((MazeState) currentState).getY()-endy)*(((MazeState) currentState).getY()-endy));
    }
}
